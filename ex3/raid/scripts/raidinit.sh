...
yes | sudo mdadm --create --verbose /dev/md127 -l 1 -n 5 /dev/sd{b,c,d,e,f}
...
for i in $(seq 1 5); do sudo mount /dev/md127p$i /raid/part$i; done
for i in $(seq 1 5); do sudo echo "/dev/md127p$i      /raid/part$i     ext4    defaults    1 2" >> /etc/fstab; done